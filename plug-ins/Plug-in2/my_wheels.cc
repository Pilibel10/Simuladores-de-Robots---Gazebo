...
namespace gazebo
{
    ...
    public:
        void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
        {
            ...
            double minY = 1.5;
            double maxY = 16.0;
            double randX = (maxX - minX) * ((double)rand() / (double)RAND_MAX) + minX;
            double randY = (maxY - minY) * ((double)rand() / (double)RAND_MAX) + minY;
    
            targetPosition = ignition::math::Vector3d(randX, randY, 0); // Cambia la posición final según sea necesario
	     a=1;
       ...
       // Check if within orientation tolerance
            ...
            printf("Posición actual: %f %f %f\n Objetivo: %f %f 0.0\n Distancia al objetivo: %f \n", pose.Pos().X(), pose.Pos().Y(), pose.Pos().Z(), targetPosition.X(), targetPosition.X(), distanceToTarget);
